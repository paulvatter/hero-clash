...
def show_account_panel():
    st.sidebar.header("Spieler-Account")
    if st.session_state.account is None:
        mode = st.sidebar.radio("Aktion", ["Anmelden", "Registrieren"], horizontal=True)
        st.session_state.auth_mode = "login" if mode == "Anmelden" else "register"

        username = st.sidebar.text_input("Benutzername")
        password = st.sidebar.text_input("Passwort", type="password")
        if st.sidebar.button("Los geht's"):
            if st.session_state.auth_mode == "login":
                ...
                if ok:
                    ...
                    if token_ok:
                        ...
                        if ok2:
                            st.session_state.account = account_data
                            st.sidebar.success("Account erstellt und angemeldet")
                            st.rerun()
                        else:
                            st.sidebar.error(account_data)
                    else:
                        st.sidebar.error(token_result)
                else:
                    st.sidebar.error(result)
    else:
        st.sidebar.success(f"Angemeldet als {st.session_state.account['username']}")
        st.sidebar.write(f"Coins: {st.session_state.account.get('coins', 0)}")
        st.sidebar.write(f"Siege: {st.session_state.account.get('wins', 0)}")
        ...
        for item in st.session_state.store_items:
            ...
            if col2.button("Kauf", key=f"buy_{item['name']}"):
                if st.session_state.account.get("coins", 0) >= item["price"]:
                    ...
                    st.sidebar.warning("Nicht genug Coins")
