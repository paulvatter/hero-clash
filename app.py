...
body{background:#07071a;font-family:'Segoe UI',sans-serif;color:#fff;user-select:none;overflow:hidden}
#wrap{border-radius:10px;overflow:hidden;width:100%;height:100vh;display:flex;flex-direction:column}
#hud{display:flex;align-items:center;gap:8px;padding:6px 12px;background:rgba(0,0,0,.7);border-bottom:1px solid rgba(255,255,255,.08);flex-shrink:0}
#hud .hname{font-size:15px;font-weight:700;color:#FFD700;min-width:80px}
.hpbar{flex:1;min-width:70px;height:8px;background:rgba(255,255,255,.12);border-radius:4px;overflow:hidden}
.hpfill{height:100%;border-radius:4px;transition:width .15s;background:#4CAF50}
#skills-bar{display:flex;gap:5px;padding:4px 10px;background:rgba(0,0,0,.5);border-bottom:1px solid rgba(255,255,255,.06);align-items:center;flex-shrink:0;flex-wrap:wrap}
.skbtn{padding:3px 8px;border-radius:6px;border:1.5px solid rgba(255,255,255,.18);background:rgba(255,255,255,.05);color:#eee;font-size:10px;font-weight:700;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:1px}
.skbtn.ready{border-color:#FFD700;color:#FFD700}
.skbtn.off{opacity:.38;cursor:not-allowed}
#cvwrap{position:relative;flex:1;overflow:hidden}
canvas{display:block;cursor:crosshair;width:100%;height:100%}
#focusbanner{position:absolute;inset:0;background:rgba(0,0,0,.55);display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:20}
#focusbanner span{background:#FFD700;color:#1a1000;font-size:15px;font-weight:900;padding:10px 28px;border-radius:9px;pointer-events:none}
#msg{position:absolute;top:38%;left:50%;transform:translate(-50%,-50%);font-size:32px;font-weight:900;color:#FFD700;text-shadow:2px 2px 0 #c60;pointer-events:none;text-align:center;transition:opacity .3s;white-space:nowrap;z-index:10}
#sel{position:absolute;inset:0;background:rgba(5,5,20,.97);display:flex;flex-direction:column;align-items:center;padding:14px;overflow-y:auto;z-index:30}
#sel h2{font-size:26px;font-weight:900;color:#FFD700;letter-spacing:2px;margin-bottom:5px}
#sel .hint{font-size:10px;color:#888;margin-bottom:9px;text-align:center}
#hgrid{display:grid;grid-template-columns:repeat(5,1fr);gap:6px;width:100%;max-width:700px;margin-bottom:8px}
.hcard{background:rgba(255,255,255,.05);border:1.5px solid rgba(255,255,255,.1);border-radius:8px;padding:7px 4px;text-align:center;cursor:pointer;transition:transform .1s,border-color .1s}
.hcard:hover{transform:scale(1.04);border-color:rgba(255,255,255,.3)}
.hcard.sel{border-color:#FFD700;background:rgba(255,215,0,.1)}
.hcard .ci{font-size:22px}
.hcard .cn{font-size:9px;font-weight:700;margin-top:2px}
.hcard .cr{font-size:8px;color:#aaa}
#ptxt{font-size:10px;color:#ccc;text-align:center;min-height:20px;margin-bottom:8px;max-width:600px}
#gobtn{font-size:13px;font-weight:900;padding:7px 24px;background:#FFD700;color:#1a1000;border:none;border-radius:8px;cursor:pointer;opacity:.4;pointer-events:none}
...
<div id="wrap">
  <div id="hud">
    ...
    <span style="font-size:11px;color:#aaa;gap:6px;display:flex">
      <span>⭐<span id="score">0</span></span>
      <span>Welle <span id="wave">1</span>/3</span>
      <span>👾<span id="ecount">0</span></span>
    </span>
    <button onclick="openSel()" style="margin-left:auto;font-size:9px;font-weight:700;padding:3px 8px;border-radius:5px;border:1px solid rgba(255,255,255,.15);background:transparent;color:#777;cursor:pointer">Held ↺</button>
  </div>
  <div id="skills-bar">
    <span style="font-size:9px;color:#555">WASD · LMB zielen · RMB schiessen · 1-4 Fähigkeiten</span>
    <div style="margin-left:auto;display:flex;gap:4px" id="skbar"></div>
  </div>
  <div id="cvwrap">
    <canvas id="cv" tabindex="0"></canvas>
    <div id="focusbanner"><span>▶ Hier klicken zum Spielen</span></div>
    <div id="msg" style="opacity:0"></div>
    <div id="sel">
      <h2>⚡ HERO CLASH ⚡</h2>
      <div class="hint">Klick auf das Spielfeld · LMB zielen · RMB schiessen · WASD laufen · 1-4 Fähigkeiten</div>
      <div id="hgrid"></div>
...
CV.addEventListener('click',()=>{CV.focus();banner.style.display='none';});
banner.addEventListener('click',()=>{CV.focus();banner.style.display='none';});
CV.addEventListener('blur',()=>{if(G&&!G.gameOver&&!G.won&&document.getElementById('sel').style.display==='none'){banner.style.display='flex';if(G.keys)G.keys={};mDown.l=false;mDown.r=false;}});
...
function genWorld(){
  ...
  for(let tx=0;tx<WW;tx+=40)for(let ty=0;ty<WH;ty+=40){const n=noise2D(tx,ty,1);terrain.push({x:tx,y:ty,col:n<.32?'#1a472a':n<.55?'#2d6a4f':n<.72?'#40916c':'#52b788'});}
...
function isSolid(x,y,r){r=r||12;for(const o of wObjs){if(o.type==='tree'&&Math.hypot(x-o.wx,y-o.wy)<o.r*.65+r*.5)return true;if(o.type==='rock'&&Math.hypot(x-o.wx,y-o.wy)<o.r+r*.4)return true;if(o.type==='building'&&Math.abs(x-o.wx)<o.w/2+r&&Math.abs(y-o.wy)<o.h/2+r)return true;if(o.type==='water'&&((x-o.wx)/o.rx)**2+((y-o.wy)/o.ry)**2<1)return true;}return x<20||x>WW-20||y<20||y>WH-20;}

const HEROES=[
  {name:'Starlance',icon:'🦸',role:'Krieger',hcol:'#3949ab',bpCol:'#c5cae9',hp:100,spd:2.8,bColor:'#FFD700',bDmg:18,bSpd:9,passive:'Jeder 3. Schuss kritisch +50%',skills:[{name:'Schildwall',icon:'🛡️',key:'1',cd:0,maxCd:300,use:g=>{g.p.shielded=150;showMsg('🛡️ Schildwall!')}},{name:'Sternstoß',icon:'🌟',key:'2',cd:0,maxCd:180,use:g=>burst(5,'#FFD700',22)},{name:'Nova',icon:'💥',key:'3',cd:0,maxCd:420,use:g=>doAoe(110,55,'#FFD700')},{name:'Heilung',icon:'💊',key:'4',cd:0,maxCd:360,use:g=>{g.p.hp=Math.min(g.p.maxHp,g.p.hp+25);showMsg('💊 +25 HP!')}}],init:s=>{s.critCount=0}},
  {name:'Pyra',icon:'🔥',role:'Angriff',hcol:'#bf360c',bpCol:'#ffccbc',hp:85,spd:3.1,bColor:'#ff5722',bDmg:22,bSpd:10,passive:'Schüsse verbrennen Feinde',skills:[{name:'Hitzeschild',icon:'♨️',key:'1',cd:0,maxCd:280,use:g=>{g.p.shielded=180;showMsg('♨️ Hitzeschild!')}},{name:'Feuerball',icon:'🌋',key:'2',cd:0,maxCd:150,use:g=>bigShot('#ff1744',40,6)},{name:'Feuerwand',icon:'🔴',key:'3',cd:0,maxCd:320,use:g=>doAoe(85,40,'#ff5722')},{name:'Inferno',icon:'☄️',key:'4',cd:0,maxCd:500,use:g=>doAoe(170,80,'#ff1744')}],init:s=>{}},
  {name:'Volt',icon:'⚡',role:'Schnell',hcol:'#f57f17',bpCol:'#fff9c4',hp:80,spd:4.2,bColor:'#fdd835',bDmg:14,bSpd:13,passive:'Schnellster Held, schnelle Schüsse',skills:[{name:'Dash',icon:'💨',key:'1',cd:0,maxCd:200,use:g=>{g.p.dodgeCd=100;showMsg('💨 Dash!')}},{name:'Salve',icon:'🌩️',key:'2',cd:0,maxCd:120,use:g=>burst(8,'#fdd835',14)},{name:'Donnerkreis',icon:'🔵',key:'3',cd:0,maxCd:260,use:g=>radial(12,'#fdd835',14)},{name:'Sturm',icon:'🌪️',key:'4',cd:0,maxCd:400,use:g=>radial(20,'#fffde7',18)}],init:s=>{s.dodgeCd=0}},
  {name:'Glacius',icon:'❄️',role:'Kontrolle',hcol:'#0277bd',bpCol:'#b3e5fc',hp:90,spd:2.6,bColor:'#81d4fa',bDmg:16,bSpd:8,passive:'Treffer verlangsamen Feinde',skills:[{name:'Eisschild',icon:'🧊',key:'1',cd:0,maxCd:260,use:g=>{g.p.shielded=160;showMsg('🧊 Eisschild!')}},{name:'Eissalve',icon:'❄️',key:'2',cd:0,maxCd:140,use:g=>burst(5,'#81d4fa',16)},{name:'Blizzard',icon:'🌨️',key:'3',cd:0,maxCd:320,use:g=>doFreezeAll()},{name:'Eisfeld',icon:'💎',key:'4',cd:0,maxCd:420,use:g=>doAoe(120,60,'#29b6f6')}],init:s=>{}},
  {name:'Terra',icon:'🌿',role:'Tank',hcol:'#2e7d32',bpCol:'#c8e6c9',hp:150,spd:2.2,bColor:'#a5d6a7',bDmg:20,bSpd:7,passive:'Regeneriert 1 HP/s, viel Leben',skills:[{name:'Panzer',icon:'🪨',key:'1',cd:0,maxCd:320,use:g=>{g.p.shielded=240;g.p.armor=true;showMsg('🪨 Wurzelpanzer!')}},{name:'Stein',icon:'🪨',key:'2',cd:0,maxCd:160,use:g=>bigShot('#8d6e63',35,5)},{name:'Ranken',icon:'🌿',key:'3',cd:0,maxCd:280,use:g=>doAoe(90,45,'#66bb6a')},{name:'Erdbeben',icon:'🌍',key:'4',cd:0,maxCd:480,use:g=>doAoe(160,80,'#66bb6a')}],init:s=>{s.regenTimer=0}},
  {name:'Phantom',icon:'👻',role:'Assassin',hcol:'#6a1b9a',bpCol:'#e1bee7',hp:75,spd:3.5,bColor:'#ce93d8',bDmg:28,bSpd:11,passive:'30% Chance Treffer zu ghosten',skills:[{name:'Unsichtbar',icon:'🌫️',key:'1',cd:0,maxCd:300,use:g=>{g.p.invisible=180;showMsg('🌫️ Unsichtbar!')}},{name:'Blitz',icon:'👻',key:'2',cd:0,maxCd:140,use:g=>burst(4,'#e040fb',30)},{name:'Seelenraub',icon:'💜',key:'3',cd:0,maxCd:300,use:g=>doDrain()},{name:'Geistersturm',icon:'🌑',key:'4',cd:0,maxCd:440,use:g=>radial(16,'#ce93d8',28)}],init:s=>{}},
  {name:'Graviton',icon:'🌀',role:'Kontrolle',hcol:'#283593',bpCol:'#c5cae9',hp:88,spd:2.7,bColor:'#9fa8da',bDmg:17,bSpd:9,passive:'AOE saugt Feinde an',skills:[{name:'Schwarzes Loch',icon:'🕳️',key:'1',cd:0,maxCd:280,use:g=>doBlackHole()},{name:'Puls',icon:'🔵',key:'2',cd:0,maxCd:130,use:g=>burst(4,'#7986cb',17)},{name:'Gravitonsog',icon:'🛸',key:'3',cd:0,maxCd:380,use:g=>doBlackHole()},{name:'Zeitwelle',icon:'⏳',key:'4',cd:0,maxCd:460,use:g=>doFreezeAll()}],init:s=>{}},
  {name:'Solara',icon:'☀️',role:'Support',hcol:'#e65100',bpCol:'#fff8e1',hp:90,spd:2.9,bColor:'#ffe082',bDmg:16,bSpd:10,passive:'+20% Schaden wenn HP > 70%',skills:[{name:'Lichtschild',icon:'💛',key:'1',cd:0,maxCd:260,use:g=>{g.p.shielded=150;showMsg('💛 Lichtschild!')}},{name:'Lichtstrahl',icon:'🌞',key:'2',cd:0,maxCd:110,use:g=>doLaser()},{name:'Sonnenwelle',icon:'✨',key:'3',cd:0,maxCd:300,use:g=>doAoe(95,50,'#ffb300')},{name:'Supernova',icon:'🌟',key:'4',cd:0,maxCd:500,use:g=>doAoe(210,100,'#FFD700')}],init:s=>{}},
  {name:'Stahlherz',icon:'🦾',role:'Tank',hcol:'#37474f',bpCol:'#eceff1',hp:140,spd:2.3,bColor:'#cfd8dc',bDmg:24,bSpd:8,passive:'Metallkörper: -3 Schaden, kein Rückstoß',skills:[{name:'Titanpanzer',icon:'🛡️',key:'1',cd:0,maxCd:280,use:g=>{g.p.shielded=300;g.p.armor=true;showMsg('🛡️ Titanpanzer!')}},{name:'Stahlwalze',icon:'⚙️',key:'2',cd:0,maxCd:160,use:g=>bigShot('#b0bec5',40,5)},{name:'Metallsturm',icon:'💠',key:'3',cd:0,maxCd:320,use:g=>radial(10,'#cfd8dc',24)},{name:'Stahlsturm',icon:'⚙️',key:'4',cd:0,maxCd:420,use:g=>doAoe(140,70,'#90a4ae')}],init:s=>{}},
  {name:'Nox',icon:'🌑',role:'Dunkel',hcol:'#4527a0',bpCol:'#d1c4e9',hp:82,spd:3.1,bColor:'#b39ddb',bDmg:20,bSpd:10,passive:'Unter 40% HP: +40% Schaden',skills:[{name:'Schattenform',icon:'👁️',key:'1',cd:0,maxCd:260,use:g=>{g.p.invisible=150;showMsg('👁️ Schattenform!')}},{name:'Dunkelwelle',icon:'🖤',key:'2',cd:0,maxCd:140,use:g=>burst(5,'#7c4dff',20)},{name:'Schattensturm',icon:'🌑',key:'3',cd:0,maxCd:300,use:g=>radial(14,'#7c4dff',20)},{name:'Apokalypse',icon:'💀',key:'4',cd:0,maxCd:500,use:g=>doAoe(190,95,'#7c4dff')}],init:s=>{}}
];
...
CV.addEventListener('mousedown',e=>{e.preventDefault();CV.focus();if(e.button===0)mDown.l=true;if(e.button===2)mDown.r=true;});
CV.addEventListener('mouseup',e=>{if(e.button===0)mDown.l=false;if(e.button===2)mDown.r=false;});
CV.addEventListener('contextmenu',e=>e.preventDefault());
CV.addEventListener('keydown',e=>{if(!G||!G.keys)return;G.keys[e.key]=true;if(e.key==='1')useSkill(0);if(e.key==='2')useSkill(1);if(e.key==='3')useSkill(2);if(e.key==='4')useSkill(3);if([' ','ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.key))e.preventDefault();});
CV.addEventListener('keyup',e=>{if(G&&G.keys)G.keys[e.key]=false;});

function openSel(){if(animId){cancelAnimationFrame(animId);animId=null;}mDown.l=false;mDown.r=false;if(G&&G.keys)G.keys={};document.getElementById('sel').style.display='flex';buildGrid();}
function buildGrid(){
...
function startGame(){if(selIdx===null)return;document.getElementById('sel').style.display='none';initGame(selIdx);CV.focus();banner.style.display='none';}
...
function bakeTerrainCanvas(){
  ...
  wObjs.filter(o=>o.type==='path').forEach(o=>{tc.fillStyle='#8B7355';if(o.horiz)tc.fillRect(0,o.pos-o.width/2,WW,o.width);else tc.fillRect(o.pos-o.width/2,0,o.width,WH);tc.fillStyle='#9E8B6A';for(let i=0;i<(o.horiz?WW:WH);i+=14){const px=o.horiz?i:o.pos+(Math.random()-.5)*o.width*.7,py=o.horiz?o.pos+(Math.random()-.5)*o.width*.7:i;tc.beginPath();tc.arc(px,py,1.5,0,Math.PI*2);tc.fill();}});
  wObjs.filter(o=>o.type==='water').forEach(o=>{const g2=tc.createRadialGradient(o.wx,o.wy,0,o.wx,o.wy,Math.max(o.rx,o.ry));g2.addColorStop(0,'#1565c0');g2.addColorStop(.7,'#1976d2');g2.addColorStop(1,'#0d47a1');tc.fillStyle=g2;tc.beginPath();tc.ellipse(o.wx,o.wy,o.rx,o.ry,0,0,Math.PI*2);tc.fill();tc.strokeStyle='#42a5f5';tc.lineWidth=1.5;tc.stroke();tc.strokeStyle='rgba(255,255,255,.15)';tc.lineWidth=1;for(let r=10;r<Math.min(o.rx,o.ry)*.7;r+=15){tc.beginPath();tc.ellipse(o.wx,o.wy,r,r*.6,0,0,Math.PI*2);tc.stroke();}});
  wObjs.filter(o=>o.type==='fence').forEach(o=>{tc.strokeStyle='#795548';tc.lineWidth=2;tc.beginPath();if(o.horiz){tc.moveTo(o.x,o.y);tc.lineTo(o.x+o.len,o.y);}else{tc.moveTo(o.x,o.y);tc.lineTo(o.x,o.y+o.len);}tc.stroke();tc.fillStyle='#6d4c41';for(let i=0;i<=o.len;i+=18){const px=o.horiz?o.x+i:o.x,py=o.horiz?o.y:o.y+i;tc.fillRect(px-2,py-5,4,10);}});
}
...
function initGame(idx){
  ...
  G={hero,p:{wx:px,wy:py,hp:hero.hp,maxHp:hero.hp,spd:hero.spd,shielded:0,invisible:0,armor:false,regenTimer:0,shotCd:0,critCount:0,dodgeCd:0,legAng:0,walkAnim:0,moving:false},
     bullets:[],enemies:[],particles:[],keys:{},wave:1,score:0,gameOver:false,won:false};
  hero.init(G);cam.x=px-W/2;cam.y=py-H/2;
  spawnWave(1);buildSkBar();updateHud();
...
function buildSkBar(){
  ...
  G.hero.skills.forEach((sk,i)=>{
    ...
    b.addEventListener('mousedown',e=>{e.preventDefault();useSkill(i);CV.focus();});
    b.innerHTML='<span style="font-size:13px">'+sk.icon+'</span><span>['+sk.key+'] '+sk.name+'</span><span id="scd'+i+'" style="font-size:9px;opacity:.7">Bereit</span>';
    sb.appendChild(b);
  });
}
function spawnWave(w){
  ...
  for(let i=0;i<count;i++){
    ...
    G.enemies.push({wx:ex,wy:ey,hp,maxHp:hp,spd:t==='fast'?1.4:0.8,type:t,shotTimer:t==='shooter'?180:99999,frozen:0,burned:0,legAng:0,walkAnim:0,col:t==='grunt'?'#c62828':t==='tank'?'#546e7a':t==='fast'?'#e65100':'#6a1b9a'});
  }
}
...
function update(){
  ...
  let dx=0,dy=0;const spd=p.spd*(p.dodgeCd>0?1.9:1);
  ...
  p.moving=!!(dx||dy);
  if(p.moving){p.walkAnim+=.2;p.legAng=Math.sin(p.walkAnim)*.45;}else{p.legAng*=.8;p.walkAnim*=.9;}
  ...
  if(p.dodgeCd>0)p.dodgeCd--;if(p.shotCd>0)p.shotCd--;
  if(h.name==='Terra'){p.regenTimer++;if(p.regenTimer>=60){p.regenTimer=0;p.hp=Math.min(p.maxHp,p.hp+1);}}
  if(mDown.r&&p.shotCd<=0){firePlayerBullet();p.shotCd=18;}
  h.skills.forEach((sk,i)=>{if(sk.cd>0){sk.cd--;const b=document.getElementById('sk'+i),s=document.getElementById('scd'+i);if(b){b.classList.toggle('ready',sk.cd===0);b.classList.toggle('off',sk.cd>0);}if(s)s.textContent=sk.cd===0?'Bereit':Math.ceil(sk.cd/60)+'s';}});
  G.bullets=G.bullets.filter(b=>{
    ...
    if(b.friendly){
      ...
      for(let i=G.enemies.length-1;i>=0;i--){
        const e=G.enemies[i];if(Math.hypot(b.wx-e.wx,b.wy-e.wy)<14){
          let dmg=b.dmg;if(h.name==='Nox'&&p.hp/p.maxHp<.4)dmg=Math.round(dmg*1.4);if(h.name==='Solara'&&p.hp/p.maxHp>.7)dmg=Math.round(dmg*1.2);
          if(b.burn)e.burned=120;if(b.slow)e.frozen=120;e.hp-=dmg;parts(e.wx,e.wy,b.col,5);
...
function draw(){
  ...
  G.bullets.forEach(b=>{const bx=sxf(b.wx),by=syf(b.wy);if(bx<-10||bx>W+10||by<-10||by>H+10)return;C.save();C.translate(bx,by);C.rotate(b.ang);C.fillStyle=b.col;if(b.pierce){C.fillRect(-11,-4,22,8);}else{C.beginPath();C.ellipse(0,0,8,4,0,0,Math.PI*2);C.fill();}C.globalAlpha=.25;C.beginPath();C.arc(0,0,10,0,Math.PI*2);C.fillStyle=b.col;C.fill();C.restore();});
  [...G.enemies].sort((a,b)=>a.wy-b.wy).forEach(e=>{if(sxf(e.wx)>-50&&sxf(e.wx)<W+50&&syf(e.wy)>-50&&syf(e.wy)<H+50)drawEnemy(e);});
  const p=G.p;drawAvatar(sxf(p.wx),syf(p.wy),aimAng,G.hero.hcol,G.hero.bpCol,p.invisible>0,p.shielded,p.legAng,G.hero.icon);
  if(mDown.l){C.save();C.strokeStyle=G.hero.bColor;C.lineWidth=1;C.globalAlpha=.25;C.setLineDash([5,6]);C.beginPath();C.moveTo(sxf(p.wx),syf(p.wy));C.lineTo(sxf(p.wx)+Math.cos(aimAng)*300,syf(p.wy)+Math.sin(aimAng)*300);C.stroke();C.restore();}
  C.save();C.strokeStyle=mDown.l?G.hero.bColor:'rgba(255,255,255,.4)';C.lineWidth=1.5;C.beginPath();C.arc(mX,mY,7,0,Math.PI*2);C.stroke();C.beginPath();C.moveTo(mX-11,mY);C.lineTo(mX+11,mY);C.moveTo(mX,mY-11);C.lineTo(mX,mY+11);C.stroke();C.restore();
  // minimap
  const mmw=90,mmh=68,mmx=W-mmw-8,mmy=H-mmh-8;
...
function updateHud(){const p=G.p;document.getElementById('hpfill').style.width=(p.hp/p.maxHp*100)+'%';document.getElementById('hptxt').textContent=Math.max(0,Math.round(p.hp))+'/'+p.maxHp;document.getElementById('score').textContent=G.score;document.getElementById('wave').textContent=G.wave;document.getElementById('ecount').textContent=G.enemies.length;document.getElementById('hname').textContent=G.hero.icon+' '+G.hero.name;const pct=p.hp/p.maxHp;document.getElementById('hpfill').style.background=pct>.5?'#4CAF50':pct>.25?'#ffb300':'#ef5350';}
...
# Streamlit layout: fullscreen via iframe
st.markdown("""
<style>
    /* Streamlit padding entfernen für Vollbild-Gefühl */
...
html_file = Path("hero_clash_game.html")
